...
// Add System namespace to use EventArgse
using System;
...
public class AppsFlyerObjectScript : MonoBehaviour, IAppsFlyerConversionData
{
    ...
    private int _avatarPartCategory_01;
    private int _avatarPartCategory_02;

    #endregion

    #region Game Mechanics Properties

    public int AvatarPartCategory_01
    {
        get => _avatarPartCategory_01;
        set => _avatarPartCategory_01 = value;
    }

    public int AvatarPartCategory_02
    {
        get => _avatarPartCategory_02;
        set => _avatarPartCategory_02 = value;
    }
    ...
    //public int OnReceivedDeepLink { get; set; }
    ...
    void Start()
    {
        ...
        AppsFlyer.setAppInviteOneLinkID("Wabz");
    ...
    public void onConversionDataSuccess(string conversionData)
    {
        ...
        if (afStatus == "Non-organic" && isFirstLaunch)
        {
            ...
            if (!_deferred_deep_link_processed_flag)
            {
                ...
                if (_conversionDataDictionary.TryGetValue("partIndex", out var partIndexObj))
                {
                    if (int.TryParse(partIndexObj?.ToString(), out var partIndex))
                    {
                        _partIndex = partIndex;
                        _deepLinkParamsDictionary.Add("PartIndex", _partIndex);
    ...
    In this app:
    deep_link_value is 아바타 파츠 index 값 
    deep_link_sub1,2 is 아바타 카테고리
    ...
    **/
    private void ParseDeepLinkParams(Dictionary<string, object> deepLinkParamsDictionary)
    {
        ...
        {
            _partIndex = deepLinkValue;
            _deepLinkParamsDictionary.Add("PartIndex", _partIndex);
        }
        // **only for Legacy Links users** if not, search for the custom param that replaces the deep_link_value.
        else if (deepLinkParamsDictionary.TryGetValue("partIndex", out var startLevelObj) && int.TryParse(startLevelObj?.ToString(), out var startLevel))
        {
            _partIndex = startLevel;
            _deepLinkParamsDictionary.Add("PartIndex", _partIndex);
        }
        // **only for Legacy Links users**
        else
        ...
        if (deepLinkParamsDictionary.TryGetValue("deep_link_sub1", out var avatarPartCategory_01))
        {
            if (int.TryParse(avatarPartCategory_01?.ToString(), out var avatarpartCategory_01))
            {
                _avatarPartCategory_01 = avatarpartCategory_01;
                _deepLinkParamsDictionary.Add("AvatarPartCategory_01", _avatarPartCategory_01);
            }
        }

        if (deepLinkParamsDictionary.TryGetValue("deep_link_sub2", out var avatarPartCategory_02))
        {
            if (int.TryParse(avatarPartCategory_02?.ToString(), out var avatarpartCategory_02))
            {
                _avatarPartCategory_02 = avatarpartCategory_02;
                _deepLinkParamsDictionary.Add("AvatarPartCategory_02", _avatarPartCategory_02);
            }
        }
        ...
        //    _deepLinkParamsDictionary.Add("AvatarPartCategory_03", referrerName);
        //}
    }
